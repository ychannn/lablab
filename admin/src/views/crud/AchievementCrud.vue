<template>
  <div class="crud-page">
    ...
    <table class="data-table">
      <thead>
        <tr><th>ID</th><th>内容</th><th width="140">操作</th></tr>
      </thead>
      <tbody>
    ...
    <div v-if="showModal" class="modal-mask" @click.self="showModal = false">
      <div class="modal">
        <h3>{{ editId ? "编辑" : "新增" }}成果奖项</h3>
        <div class="form-group"><label>内容</label><textarea v-model="form.content" class="form-input" rows="4"></textarea></div>
...
export default {
  ...
  methods: {
    async load(p) {
      const data = await request("/achievement/page?pageNum=" + p + "&pageSize=" + this.pageSize)
    ...
    openAdd() { this.editId = null; this.form = { content: "" }; this.showModal = true },
    openEdit(row) { this.editId = row.id; this.form = { content: row.content || "" }; this.showModal = true },
    async submit() {
      this.saving = true
      try {
        const payload = { content: this.form.content }
        if (this.editId) { payload.id = this.editId; await request("/achievement/update", { method: "PUT", body: JSON.stringify(payload) }) }
    ...
    async remove(id) {
      ...
      try { await request("/achievement/delete/" + id, { method: "DELETE" }); this.load(this.pageNum) }
