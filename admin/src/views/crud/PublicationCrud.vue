<template>
  <div class="crud-page">
      ...
      <div class="filter-wrap">
        <form class="filter-row" @submit.prevent="onSearch">
          ...
          <div class="filter-time-group" ref="dtWrap">
            <div class="dt-wrap">
              <button type="button" class="dt-trigger" :class="{ filled: filter.pubStartDate }" @click.stop="dtpOpen = dtpOpen === 'start' ? null : 'start'" title="开始">
                ...
                <span class="dt-text">{{ pubStartDisplay }}</span>
              </button>
              <div v-show="dtpOpen === 'start'" class="dt-popover" @click.stop>
                <div class="dt-popover-title">开始日期</div>
                <label class="date-only-wrap" for="pub-dtp-start" @click="openDatePicker">
                  <span class="date-only-text">{{ filter.pubStartDate ? dateLabel(filter.pubStartDate) : '请选择日期' }}</span>
                  <input id="pub-dtp-start" v-model="filter.pubStartDate" type="date" class="date-only-input" />
                </label>
                <div class="dt-popover-actions"><button type="button" class="dt-btn-clear" @click="clearDtp('pubStart')">清空</button><button type="button" class="dt-btn-ok" @click="dtpOpen = null">确定</button></div>
              </div>
            </div>
            <span class="filter-sep">至</span>
            <div class="dt-wrap">
              <button type="button" class="dt-trigger" :class="{ filled: filter.pubEndDate }" @click.stop="dtpOpen = dtpOpen === 'end' ? null : 'end'" title="结束">
                <span class="dt-icon"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg></span>
                <span class="dt-text">{{ pubEndDisplay }}</span>
              </button>
              <div v-show="dtpOpen === 'end'" class="dt-popover" @click.stop>
                <div class="dt-popover-title">结束日期</div>
                <label class="date-only-wrap" for="pub-dtp-end" @click="openDatePicker">
                  <span class="date-only-text">{{ filter.pubEndDate ? dateLabel(filter.pubEndDate) : '请选择日期' }}</span>
                  <input id="pub-dtp-end" v-model="filter.pubEndDate" type="date" class="date-only-input" />
                </label>
                <div class="dt-popover-actions"><button type="button" class="dt-btn-clear" @click="clearDtp('pubEnd')">清空</button><button type="button" class="dt-btn-ok" @click="dtpOpen = null">确定</button></div>
              </div>
            </div>
          </div>
    ...
    <table class="data-table">
      <thead>
        <tr><th>ID</th><th>内容</th><th>发表时间</th><th width="140">操作</th></tr>
      </thead>
      <tbody>
        <tr v-for="row in list" :key="row.id">
          <td>{{ row.id }}</td>
          <td>{{ (row.content || '').slice(0, 60) }}{{ (row.content || '').length > 60 ? '…' : '' }}</td>
          <td>{{ formatTime(row.publishTime) }}</td>
          <td>
...
export default {
  name: 'PublicationCrud',
  data() {
    return {
      list: [], pageNum: 1, pageSize: 10, total: 0, pages: 0,
      showModal: false, editId: null, saving: false,
      form: { content: '', publishTime: '' },
      filter: { keyword: '', pubStartDate: '', pubStartTime: '', pubEndDate: '', pubEndTime: '' },
      dtpOpen: null
    }
  },
  computed: {
    pubStartDisplay() { return this.filter.pubStartDate ? this.dateLabel(this.filter.pubStartDate) : '开始' },
    pubEndDisplay() { return this.filter.pubEndDate ? this.dateLabel(this.filter.pubEndDate) : '结束' }
  },
  mounted() {
    this.load(1)
  ...
  methods: {
    ...
    clearDtp(which) {
      if (which === 'pubStart') { this.filter.pubStartDate = ''; this.filter.pubStartTime = '' }
      else { this.filter.pubEndDate = ''; this.filter.pubEndTime = '' }
      this.dtpOpen = null
    },
    ...
    buildPageQuery(p) {
      ...
      const pubStart = this.buildDateTime(this.filter.pubStartDate, this.filter.pubStartTime, '00:00')
      if (pubStart) params.set('publishTimeStart', pubStart)
      const pubEnd = this.buildDateTime(this.filter.pubEndDate, this.filter.pubEndTime, '23:59')
      if (pubEnd) params.set('publishTimeEnd', pubEnd)
      return `/publication/page?${params.toString()}`
    },
    async load(p) {
    ...
    onResetFilter() {
      this.filter = { keyword: '', pubStartDate: '', pubStartTime: '', pubEndDate: '', pubEndTime: '' }
      this.dtpOpen = null
      this.load(1)
    },
    openAdd() {
    ...
    async submit() {
      ...
      try {
        ...
        if (this.editId) {
          ...
          await request('/publication/add', { method: 'POST', body: JSON.stringify(payload) })
        }
    ...
    async remove(id) {
      ...
      try {
        await request(`/publication/delete/${id}`, { method: 'DELETE' })
        this.load(this.pageNum)
...
.dt-trigger { display: inline-flex; align-items: center; gap: 4px; padding: 6px 10px; min-width: 72px; max-width: 100px; border: 1px solid #d9d9d9; border-radius: 6px; background: #fff; color: #595959; font-size: 12px; cursor: pointer; }
