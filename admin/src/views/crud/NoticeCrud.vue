          ...
          <div class="filter-time-group" ref="dtWrap">
            ...
            <div class="dt-wrap">
              <button type="button" class="dt-trigger" :class="{ filled: filter.timeEndDate }" @click.stop="dtpOpen = dtpOpen === 'end' ? null : 'end'" title="结束时间">
                <span class="dt-icon" aria-hidden="true"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg></span>
                <span class="dt-text">{{ filter.timeEndDate ? dateLabel(filter.timeEndDate) : '结束' }}</span>
              </button>
...
export default {
  ...
  methods: {
    ...
    buildPageQuery(p) {
      ...
      return `/notice/page?${params.toString()}`
    },
    async load(p) {
    ...
    openAdd() {
      this.editId = null
      this.form = { title: '', content: '', time: new Date().toISOString().slice(0, 10) }
      this.showModal = true
    },
    openEdit(row) {
      this.editId = row.id
      const timeStr = (row.time || row.createTime || '')
      this.form = {
        title: row.title || '',
        content: row.content || '',
        time: timeStr ? String(timeStr).slice(0, 10) : ''
      }
      this.showModal = true
    },
    async submit() {
      ...
      try {
        ...
        if (this.editId) {
          ...
          await request('/notice/update', { method: 'PUT', body: JSON.stringify(payload) })
        } else {
          await request('/notice/add', { method: 'POST', body: JSON.stringify(payload) })
        }
    ...
    async remove(id) {
      ...
      try {
        await request(`/notice/delete/${id}`, { method: 'DELETE' })
        this.load(this.pageNum)
      } catch (e) {
...
.modal { background: #fff; border-radius: 8px; padding: 28px; min-width: 560px; width: 90vw; max-width: 900px; max-height: 92vh; overflow-y: auto; }
...
.content-group .content-field { min-width: 100%; resize: vertical; min-height: 120px; }
